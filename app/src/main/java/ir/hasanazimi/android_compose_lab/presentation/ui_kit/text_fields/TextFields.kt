...
import ir.hasanazimi.android_compose_lab.presentation.ui_kit.text_fields.text_fields_validators.ValidationType
import ir.hasanazimi.android_compose_lab.presentation.ui_kit.text_fields.text_fields_validators.ValidatorFactory
...
fun EmailTextField(
    ...
    val config = TextFieldConfig.getConfig(InputConfigType.Email)
    ...
    if (outlineStyle){
        BaseOutlineTextField(
            ...
            enable = enable,
            config = config,
            validatorResult = ValidatorFactory.getValidator(validationType = ValidationType.Email).validate(inputData)
        ) { typingData ->
            onValueChange(
                ValidatorFactory.getValidator(validationType = ValidationType.Email)
                    .validate(typingData)
            )
        }
    }else{
